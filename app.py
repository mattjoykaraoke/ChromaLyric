...
# - Typography drawer with real-time 100% boundary checking (1920x1080 limits).
# - Global font sizing sync (default ON), isolated typeface sync (default OFF).
# - Retains always-on karaoke preview, background export tags, and strict preset handling.
# - NEW: Advanced KBS \pos(x,y) collision detection mimicking literal screen overlaps!
        ...
        for l in lines:
            ...
            if s.lower().startswith("dialogue:"):
                ...
                if event_format and len(parts) >= len(event_format):
                    ...
                    if txt:
                        # Extract exact \pos tags generated by KBS before stripping!
                        pos_match = re.search(
                            r"\\pos\(([\d.-]+)\s*,\s*([\d.-]+)\)", txt
                        )
                        pos_data = (
...
class MainWindow(QMainWindow):
    ...
    def _check_timeline_safety(
        ...
        r"""
        Uses a sweep-line algorithm with literal bounding box tracking.
        It perfectly honors \pos(x,y) coordinates set by KBS to check for text crashing together or bleeding off screen.
        """
        ...
        for st in self.doc.styles:
            ...
            # Factor in outline/shadow to perfectly measure total render width
            style_padding[st.name] = style_get_int(st, "Outline", 0) + style_get_int(
                st, "Shadow", 0
            )

        # 2. Sweep Line Algorithm Setup
        events = []
        ...
        for time, ev_type, idx in events:
            ...
            if ev_type == "start":
                ...
                for active_idx in active_diags:
                    ...
                    w = max(
                        (metrics.horizontalAdvance(l) for l in d["lines"]), default=0
                    )
                    h = metrics.height() * len(d["lines"])
                    align = style_alignments.get(d["style"], 2)
                    padding = style_padding.get(d["style"], 0)

                    if d["pos"]:
                        # The absolute position override generated by KBS!
                        ax, ay = d["pos"]

                        # Apply X Anchor Rules
                        if align in (1, 4, 7):
                            L, R = ax, ax + w
                        elif align in (3, 6, 9):
                            L, R = ax - w, ax
                        ...
                        # Add literal border/shadow padding
                        L -= padding
                        R += padding
                        T -= padding
                        B += padding

                        # Check 1: Does this text box bleed off the 1080p screen?
                        ...
                        # Fallback for standard auto-stacking ASS lines without \pos
                        align_groups.setdefault(align, []).append(
                            (d["raw_text_clean"], w + padding * 2, h + padding * 2)
                        )

                # Filter identical duplicates (KBS sweeping/shadow layers) to prevent false overlaps
                unique_rects = []
                seen_keys = set()
                for r in rects:
                ...
                # Check 2: Do any two \pos anchored bounding boxes physically crash into each other?
                for i in range(len(unique_rects)):
                    for j in range(i + 1, len(unique_rects)):
                        ...
                        # Standard intersection with a 2-pixel tolerance buffer to prevent false-positives on flush text
                        if L1 < R2 - 2 and R1 > L2 + 2 and T1 < B2 - 2 and B1 > T2 + 2:
                            return (
                                False,
                                "Overlapping lyrics detected! (Crash collision)",
                            )

                # Fallback bounds check for non-pos lines
    ...
    def check_font_boundaries(self):
        """Measures the global timeline to ensure lyrics don't crash or exceed 1920x1080"""
    ...
    def snap_to_safe_size(self):
        """Automatically shrinks the font until ALL lyrics fit without crashing or bleeding off 1080p edges."""
        if not self.doc or not self.doc.parsed_dialogues:
            return
        ...
        # Keep silently testing smaller sizes across the entire song until it fits perfectly
        while size > 1:
